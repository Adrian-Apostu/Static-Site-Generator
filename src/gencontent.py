...
def generate_pages_recursive(dir_path_content, template_path, dest_dir_path, basepath):
    for item in os.listdir(dir_path_content):
        ...
        if os.path.isfile(current_path) and Path(current_path).suffix == ".md":
            ...
            generate_page(current_path, template_path, os.path.join(dest_dir_path, new_path.name), basepath)
        elif not os.path.isfile(current_path):
            new_dst = os.path.join(dest_dir_path, item)
            generate_pages_recursive(current_path, template_path, new_dst, basepath)